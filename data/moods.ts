// export const moods = [
//     {
//       id: '1',
//       title: 'Joyful',
//       description: 'Rejoice in the Lord always; let His joy fill your heart and overflow into your life.',
//       images: [
//         '/Assets/ruth.jpg',
//         '/Assets/ruth.jpg',
//       ],
//       personalStory: 
//         'During one of my darkest times, I found a spark of joy by trusting God wholeheartedly. I realized joy isn’t tied to circumstances but rooted in His eternal promises. Let us all learn to choose joy daily by meditating on His word and leaning into His love.',
//       resources: [
//         'https://www.biblegateway.com/passage/?search=Philippians+4%3A4&version=NIV',
//         'https://www.crosswalk.com/faith/spiritual-life/what-is-biblical-joy.html',
//       ],
//     },
//     {
//       id: '2',
//       title: 'Peaceful',
//       description: 'Rest in the peace that surpasses all understanding and let God’s presence calm your storms.',
//       images: [
//         '/Assets/ruth.jpg',
//         '/Assets/ruth.jpg',
//       ],
//       personalStory: 
//         'I was overwhelmed with anxiety until I started casting my worries onto Him in prayer. It wasn’t instant, but the peace that followed was undeniable. You can also find this peace by trusting Him with your burdens.',
//       resources: [
//         'https://www.biblegateway.com/passage/?search=Philippians+4%3A7&version=NIV',
//         'https://www.desiringgod.org/articles/how-to-find-peace-in-chaos',
//       ],
//     },
//     {
//       id: '3',
//       title: 'Hopeful',
//       description: 'Let hope arise, knowing that God is faithful to His promises and His plans for you are good.',
//       images: [
//         '/Assets/ruth.jpg',
//         '/Assets/ruth.jpg',
//       ],
//       personalStory: 
//         'When life seemed hopeless, I clung to His promises and found renewed strength. His word reminded me that He works all things for good. Stay hopeful, for He has a future planned for you.',
//       resources: [
//         'https://www.biblegateway.com/passage/?search=Jeremiah+29%3A11&version=NIV',
//         'https://www.gotquestions.org/biblical-hope.html',
//       ],
//     },
//     {
//       id: '4',
//       title: 'Thankful',
//       description: 'In every situation, give thanks to the Lord, for gratitude shifts our focus from problems to His blessings.',
//       images: [
//         '/Assets/ruth.jpg',
//         '/Assets/ruth.jpg',
//       ],
//       personalStory: 
//         'Gratitude transformed my perspective. Instead of dwelling on what I lacked, I praised God for His endless provisions. Cultivating thankfulness can change your life too!',
//       resources: [
//         'https://www.biblegateway.com/passage/?search=1+Thessalonians+5%3A18&version=NIV',
//         'https://www.christianity.com/christian-life/why-gratitude-is-important.html',
//       ],
//     },
//     {
//       id: '5',
//       title: 'Broken',
//       description: 'When your heart feels shattered, trust that God is near the brokenhearted and saves those crushed in spirit.',
//       images: [
//         '/Assets/ruth.jpg',
//         '/Assets/ruth.jpg',
//       ],
//       personalStory: 
//         'In my brokenness, I felt unworthy and lost. But God reminded me that He uses broken vessels to shine His light. Surrender your pain to Him, and let Him heal you.',
//       resources: [
//         'https://www.biblegateway.com/passage/?search=Psalm+34%3A18&version=NIV',
//         'https://www.desiringgod.org/articles/healing-from-brokenness',
//       ],
//     },
//     {
//       id: '6',
//       title: 'Lonely',
//       description: 'Even when you feel alone, God is always with you. He will never leave nor forsake you.',
//       images: [
//         '/Assets/ruth.jpg',
//         '/Assets/ruth.jpg',
//       ],
//       personalStory: 
//         'Loneliness consumed me until I realized I was never truly alone. God’s presence brought comfort and purpose. Remember, He is always with you.',
//       resources: [
//         'https://www.biblegateway.com/passage/?search=Deuteronomy+31%3A6&version=NIV',
//         'https://www.gotquestions.org/loneliness.html',
//       ],
//     },
//     {
//       id: '7',
//       title: 'Forgiven',
//       description: 'Rejoice in the freedom of forgiveness through Christ. His mercy is new every morning.',
//       images: [
//         '/Assets/ruth.jpg',
//         '/Assets/ruth.jpg',
//       ],
//       personalStory: 
//         'I struggled to forgive myself for past mistakes until I grasped the depth of His forgiveness. Let His grace wash over you, and forgive yourself too.',
//       resources: [
//         'https://www.biblegateway.com/passage/?search=1+John+1%3A9&version=NIV',
//         'https://www.christianity.com/wiki/salvation/what-is-forgiveness-according-to-the-bible.html',
//       ],
//     },
//     {
//       id: '8',
//       title: 'Victorious',
//       description: 'Through Christ, you are more than a conqueror. Walk in His victory every day.',
//       images: [
//         '/Assets/ruth.jpg',
//         '/Assets/ruth.jpg',
//       ],
//       personalStory: 
//         'Challenges made me doubt myself, but through Christ, I learned that victory is already mine. Lean on His strength, and you will overcome.',
//       resources: [
//         'https://www.biblegateway.com/passage/?search=Romans+8%3A37&version=NIV',
//         'https://www.crosswalk.com/faith/spiritual-life/victory-in-jesus.html',
//       ],
//     },
//   ];


  export const moods = [
    {
      id: '1',
      title: 'Joyful',
      description: 'Rejoice in the Lord always; let His joy fill your heart and overflow into your life.',
      images: [
        '/Assets/ruth.jpg',
        '/Assets/ruth.jpg',
      ],
      personalStory: 
        'There was a season in my life when joy felt elusive. I had tied my happiness to things like career success and relationships, which often left me feeling empty. One day, I stumbled upon Philippians 4:4, which challenged me to rejoice in the Lord always. Slowly, I started shifting my focus from my circumstances to God’s unchanging love. I began starting my mornings by thanking Him for the smallest blessings: the sunrise, the sound of birds, even the ability to breathe. Over time, His joy replaced my anxiety. I learned that true joy comes from trusting His promises and walking in His presence daily.',
      verses: [
        'Philippians 4:4 - Rejoice in the Lord always. I will say it again: Rejoice!',
        'Psalm 16:11 - You make known to me the path of life; in Your presence there is fullness of joy; at Your right hand are pleasures forevermore.',
        'John 15:11 - These things I have spoken to you, that My joy may be in you, and that your joy may be full.',
      ],
      resources: [
        'https://www.biblegateway.com/passage/?search=Philippians+4%3A4&version=NIV',
        'https://www.crosswalk.com/faith/spiritual-life/what-is-biblical-joy.html',
      ],
    },
    {
      id: '2',
      title: 'Peaceful',
      description: 'Rest in the peace that surpasses all understanding and let God’s presence calm your storms.',
      images: [
        '/Assets/ruth.jpg',
        '/Assets/ruth.jpg',
      ],
      personalStory: 
        'Anxiety was my constant companion for years. I would lie awake at night, replaying my mistakes and worrying about the future. I reached a breaking point when the stress began affecting my health. A friend shared Philippians 4:6-7 with me, urging me to pray instead of worry. At first, it was hard to trust God with my burdens, but I started journaling my prayers and leaving each concern at His feet. Gradually, I felt a calmness I had never known before. That peace wasn’t tied to my situation but to the assurance that God was in control. Today, when storms arise, I anchor myself in His promises, knowing His peace will guard my heart and mind.',
      verses: [
        'Philippians 4:7 - And the peace of God, which surpasses all understanding, will guard your hearts and your minds in Christ Jesus.',
        'John 14:27 - Peace I leave with you; My peace I give to you. I do not give as the world gives. Do not let your hearts be troubled and do not be afraid.',
        'Isaiah 26:3 - You will keep in perfect peace those whose minds are steadfast, because they trust in You.',
      ],
      resources: [
        'https://www.biblegateway.com/passage/?search=Philippians+4%3A7&version=NIV',
        'https://www.desiringgod.org/articles/how-to-find-peace-in-chaos',
      ],
    },
    {
      id: '3',
      title: 'Hopeful',
      description: 'Let hope arise, knowing that God is faithful to His promises and His plans for you are good.',
      images: [
        '/Assets/ruth.jpg',
        '/Assets/ruth.jpg',
      ],
      personalStory: 
        'I once went through a season of hopelessness. Nothing seemed to go as planned, and every door I knocked on seemed to close. I felt forgotten and invisible. One night, while pouring out my frustrations to God, I opened my Bible to Jeremiah 29:11. The words reminded me that God’s plans for me were good, even if I couldn’t see them yet. I began to cling to that promise, asking Him to guide me one step at a time. As I trusted Him, opportunities began to unfold, not in my timing but in His. Hope became my anchor, reminding me that He is always faithful, even in the waiting.',
      verses: [
        'Jeremiah 29:11 - For I know the plans I have for you, declares the Lord, plans to prosper you and not to harm you, plans to give you hope and a future.',
        'Romans 15:13 - May the God of hope fill you with all joy and peace as you trust in Him, so that you may overflow with hope by the power of the Holy Spirit.',
        'Lamentations 3:22-23 - Because of the Lord’s great love we are not consumed, for His compassions never fail. They are new every morning; great is Your faithfulness.',
      ],
      resources: [
        'https://www.biblegateway.com/passage/?search=Jeremiah+29%3A11&version=NIV',
        'https://www.gotquestions.org/biblical-hope.html',
      ],
    },
    {
      id: '4',
      title: 'Thankful',
      description: 'In every situation, give thanks to the Lord, for gratitude shifts our focus from problems to His blessings.',
      images: [
        '/Assets/ruth.jpg',
        '/Assets/ruth.jpg',
      ],
      personalStory: 
        'There was a time when all I could see were the things going wrong in my life. I constantly compared myself to others and felt like I was falling short. One day, I came across 1 Thessalonians 5:18, which says to give thanks in all circumstances. At first, it felt unnatural to thank God when life felt overwhelming. But I started a gratitude journal, writing down three things I was thankful for each day. It changed everything. My perspective shifted, and I began noticing God’s blessings everywhere—His provision, His kindness, and His faithfulness. Gratitude has become my daily anthem, helping me see life through the lens of His goodness.',
      verses: [
        '1 Thessalonians 5:18 - Give thanks in all circumstances; for this is God’s will for you in Christ Jesus.',
        'Psalm 107:1 - Give thanks to the Lord, for He is good; His love endures forever.',
        'Colossians 3:17 - And whatever you do, whether in word or deed, do it all in the name of the Lord Jesus, giving thanks to God the Father through Him.',
      ],
      resources: [
        'https://www.biblegateway.com/passage/?search=1+Thessalonians+5%3A18&version=NIV',
        'https://www.christianity.com/christian-life/why-gratitude-is-important.html',
      ],
    },
    {
      id: '5',
      title: 'Broken',
      description: 'When your heart feels shattered, trust that God is near the brokenhearted and saves those crushed in spirit.',
      images: [
        '/Assets/ruth.jpg',
        '/Assets/ruth.jpg',
      ],
      personalStory: 
        'I remember the day I hit rock bottom. My heart felt shattered, and I couldn’t see a way forward. In my brokenness, I cried out to God, not even knowing what to say. Psalm 34:18 reminded me that He is near to the brokenhearted. Slowly, I felt His presence wrapping around me like a warm embrace. He didn’t take away the pain immediately, but He gave me the strength to keep going. Over time, I realized that He was using my brokenness to reshape me into something beautiful. I’ve learned that it’s in our weakest moments that His power is made perfect.',
      verses: [
        'Psalm 34:18 - The Lord is close to the brokenhearted and saves those who are crushed in spirit.',
        'Isaiah 61:1 - The Spirit of the Sovereign Lord is on me, because the Lord has anointed me to proclaim good news to the poor. He has sent me to bind up the brokenhearted, to proclaim freedom for the captives and release from darkness for the prisoners.',
        '2 Corinthians 12:9 - But He said to me, "My grace is sufficient for you, for My power is made perfect in weakness."',
      ],
      resources: [
        'https://www.biblegateway.com/passage/?search=Psalm+34%3A18&version=NIV',
        'https://www.desiringgod.org/articles/healing-from-brokenness',
      ],
    },
    {
      id: '6',
      title: 'Lonely',
      description: 'Even when you feel alone, God is always with you. He will never leave nor forsake you.',
      images: [
        '/Assets/ruth.jpg',
        '/Assets/ruth.jpg',
      ],
      personalStory: 
        'I’ve always struggled with feeling alone, especially during tough times. Even in a crowded room, I felt invisible and unimportant. One night, I cried out to God, asking why I felt so abandoned. That’s when I came across Deuteronomy 31:6, where God promises never to leave us or forsake us. Those words felt like a whisper straight to my soul. I started reminding myself daily that He was with me, even in my loneliest moments. Over time, His presence became my greatest comfort. Now, whenever loneliness creeps in, I hold on to His promise that I’m never truly alone.',
      verses: [
        'Deuteronomy 31:6 - Be strong and courageous. Do not be afraid or terrified because of them, for the Lord your God goes with you; He will never leave you nor forsake you.',
        'Psalm 23:4 - Even though I walk through the darkest valley, I will fear no evil, for You are with me; Your rod and Your staff, they comfort me.',
        'Matthew 28:20 - And surely I am with you always, to the very end of the age.',
      ],
      resources: [
        'https://www.biblegateway.com/passage/?search=Deuteronomy+31%3A6&version=NIV',
        'https://www.gotquestions.org/loneliness.html',
      ],
    },
    {
      id: '7',
      title: 'Forgiven',
      description: 'Rejoice in the freedom of forgiveness through Christ. His mercy is new every morning.',
      images: [
        '/Assets/ruth.jpg',
        '/Assets/ruth.jpg',
      ],
      personalStory: 
        'For the longest time, I carried the weight of guilt and shame for my past mistakes. I felt unworthy of God’s love, convinced that I had gone too far. One day, while reading 1 John 1:9, I realized that His forgiveness wasn’t something I needed to earn—it was a gift. I confessed my sins and felt an overwhelming sense of freedom. His grace washed over me like a river, cleansing me from my past. Now, I walk in the assurance that I am forgiven and loved, and I remind others that no sin is too great for His mercy.',
      verses: [
        '1 John 1:9 - If we confess our sins, He is faithful and just to forgive us our sins and to cleanse us from all unrighteousness.',
        'Psalm 103:12 - As far as the east is from the west, so far has He removed our transgressions from us.',
        'Lamentations 3:22-23 - Because of the Lord’s great love we are not consumed, for His compassions never fail. They are new every morning; great is Your faithfulness.',
      ],
      resources: [
        'https://www.biblegateway.com/passage/?search=1+John+1%3A9&version=NIV',
        'https://www.christianity.com/wiki/salvation/what-is-forgiveness-according-to-the-bible.html',
      ],
    },
    {
      id: '8',
      title: 'Victorious',
      description: 'Through Christ, you are more than a conqueror. Walk in His victory every day.',
      images: [
        '/Assets/ruth.jpg',
        '/Assets/ruth.jpg',
      ],
      personalStory: 
        'Life often felt like a series of battles I couldn’t win. I struggled with self-doubt and felt defeated before even trying. Then I encountered Romans 8:37, which reminded me that in Christ, I am more than a conqueror. Slowly, I began facing challenges with a different mindset, leaning on His strength instead of my own. Every victory, big or small, became a testimony of His power in my life. I’ve learned that the battle is already won in Him, and I now walk in that victory daily.',
      verses: [
        'Romans 8:37 - No, in all these things we are more than conquerors through Him who loved us.',
        '1 Corinthians 15:57 - But thanks be to God, who gives us the victory through our Lord Jesus Christ.',
        '2 Corinthians 2:14 - But thanks be to God, who in Christ always leads us in triumphal procession.',
      ],
      resources: [
        'https://www.biblegateway.com/passage/?search=Romans+8%3A37&version=NIV',
        'https://www.crosswalk.com/faith/spiritual-life/victory-in-jesus.html',
      ],
    },
  ];
    // Continue similarly with Thankful, Broken, Lonely, Forgiven, Victorious..
  